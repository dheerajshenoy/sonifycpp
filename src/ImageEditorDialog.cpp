...
: QDialog(parent)
{
    ...
    connect(m_gray_cb, &QCheckBox::checkStateChanged, this, &ImageEditorDialog::applyImageOptions);
    connect(m_invert_cb, &QCheckBox::checkStateChanged, this, &ImageEditorDialog::applyImageOptions);
    connect(m_brightness_slider, &QSlider::valueChanged, this, &ImageEditorDialog::applyImageOptions);
    ...
    connect(m_apply_btn, &QPushButton::clicked, this, [&]() {
        ...
        auto gamma = m_gamma_slider->value();
        auto grayscale = m_gray_cb->isChecked();
...
void ImageEditorDialog::setPixmap(QPixmap &pix)
{
    ...
    if (m_img.height() > 200 && m_img.width() > 200)
        m_img = m_img.scaled(200, 200, Qt::KeepAspectRatio, Qt::SmoothTransformation);
