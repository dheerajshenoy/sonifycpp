...
WaveformWidget::WaveformWidget(QWidget *parent) noexcept : QWidget(parent) {
    ...
    connect(
        m_wf_save_btn, &QPushButton::clicked, this, &WaveformWidget::savePlot);

    this->setLayout(m_wf_widget_layout);
}

void
WaveformWidget::setData(const std::vector<short> &data,
    ...
    for (uint32_t i = 0; i < size; i++) {
        ...
        if (data[i] < 0)
            y[i] = static_cast<double>(data.at(i) / 32768.0);
        else
            y[i] = static_cast<double>(data.at(i) / 32767.0);
    }
