...
// gamma 0.1 to 300
QImage Utils::changeGamma(QImage &image, int _value, int h, int w)
{
...
QImage Utils::changeBrightness(QImage &image, int value, int h, int w)
{
    for (int y = 0; y < h; ++y)
    {
        for (int x = 0; x < w; ++x)
...
QImage Utils::changeSaturation(QImage &image, int value, int h, int w)
{
    for (int y = 0; y < h; ++y) {
        for (int x = 0; x < w; ++x) {
            QColor color = image.pixelColor(x, y);
...
QImage Utils::invertColor(QImage &img, int h, int w)
{
    for(int i=0; i < w; i++)
        for(int j=0; j < h; j++)
        {
...
QImage Utils::convertToGrayscale(QImage &img, int h, int w)
{
    for(int i=0; i < w; i++)
        for(int j=0; j < h; j++)
        {
...
QImage Utils::changeContrast(QImage &image, int _value, int h, int w)
{
